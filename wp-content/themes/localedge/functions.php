...
function localedge_content_width() {
	if ( is_attachment() && wp_attachment_is_image() ) {
		$GLOBALS['content_width'] = 810;
	}
}
add_action( 'template_redirect', 'localedge_content_width' );
...
function localedge_scripts_and_styles() {
	...
	$wp_styles->add_data( 'localedge-ie-only', 'conditional', 'lt IE 9' ); // add conditional wrapper around ie stylesheet
...
function localedge_theme_support() {

	// Make theme available for translation
	load_theme_textdomain( 'localedge', get_template_directory() . '/languages' );
	...
	*/
	...
	add_theme_support( 'custom-header', array(
		...
		'header-text'=> false,
		'uploads'=> true,
		'wp-head-callback'=> 'localedge_custom_headers_callback',
		'admin-head-callback'=> '',
		'admin-preview-callback'=> '',
		)
	);
...
*/
	...
	register_nav_menus(
		array(
			'main-nav' => __( 'Main Menu', 'localedge' ),	// main nav in header
			'footer-nav' => __( 'Footer Menu', 'localedge' ) // secondary nav in footer
		)
	);
...
function localedge_custom_headers_callback() {
...
// Thumbnail sizes
//add_image_size( 'localedge-thumb-600', 600, 150, true );
...
function localedge_change_post_menu_label() {
	global $menu;
	global $submenu;
	$menu[5][0] = 'News';
...
function localedge_change_post_object_label() {
...
add_action( 'init', 'localedge_change_post_object_label' );
add_action( 'admin_menu', 'localedge_change_post_menu_label' );
*/
...
function localedge_main_nav() {
	...
	wp_nav_menu(array(
		...
		'walker'=> new localedge_walker_nav_menu
	));
} /* end scaffolding main nav */

// the footer menu (should you choose to use one)
function localedge_footer_nav() {
	wp_nav_menu(array(
		'container' => '',
		'container_class' => '',
		'menu' => '',
...
class localedge_walker_nav_menu extends Walker_Nav_Menu {
...
function localedge_register_sidebars() {
	register_sidebar(array(
		'id' => 'left-sidebar',
		'name' => __('Left Sidebar', 'localedge'),
		'description' => __('The Left (primary) sidebar used for the interior menu.', 'localedge'),
	...
	register_sidebar(array(
		'id' => 'right-sidebar',
		'name' => __('Right Sidebar', 'localedge'),
		'description' => __('The Right sidebar used for the interior call to actions.', 'localedge'),
		'before_widget' => '<div id="%1$s" class="widget %2$s">',
		'after_widget' => '</div>',
		'before_title' => '<h4 class="widgettitle">',
		'after_title' => '</h4>',
    ...
    register_sidebar(array(
		...
		'name' => __('Secondary Sidebar', 'localedge'),
		'description' => __('The secondary sidebar used for the interior call to actions.', 'localedge'),
		'before_widget' => '<div id="%1$s" class="widget %2$s">',
		'after_widget' => '</div>',
		'before_title' => '<h4 class="widgettitle">',
		'after_title' => '</h4>',
...
// Related Posts Function (call using localedge_related_posts(); )
function localedge_related_posts() {
	echo '<ul id="localedge-related-posts">';
	global $post;
	...
	if($tags) {
		...
		if($related_posts) {
			...
			echo '<li class="no_related_post">' . __( 'No Related Posts Yet!', 'localedge' ) . '</li>';
		}
	}
	wp_reset_query();
	echo '</ul>';
...
// Comment Layout
function localedge_comments($comment, $args, $depth) {
		...
		<article id="comment-<?php comment_ID(); ?>" class="clearfix">
			<header class="comment-author vcard">
				...
				*/
				...
				<?php edit_comment_link(__('(Edit)', 'localedge'),'  ','') ?>
			</header>
			<?php if ($comment->comment_approved == '0') : ?>
	   			<div class="alert info">
		  			<p><?php _e('Your comment is awaiting moderation.', 'localedge') ?></p>
		  		</div>
			<?php endif; ?>
...
// Search Form
function localedge_wpsearch($form) {
	$form = '<form role="search" method="get" id="searchform" action="' . home_url( '/' ) . '" >
	<label class="screen-reader-text" for="s">' . __('Search for:', 'localedge') . '</label>
	<input type="text" value="' . get_search_query() . '" name="s" id="s" placeholder="'.esc_attr__('Search the Site...','localedge').'" />
...
function localedge_add_first_and_last($output) {
...
add_filter('wp_nav_menu', 'localedge_add_first_and_last');

// Add "first" and "last" CSS classes to dynamic sidebar widgets. Also adds numeric index class for each widget (widget-1, widget-2, etc.)
function localedge_widget_first_last_classes($params) {
...
add_filter( 'dynamic_sidebar_params', 'localedge_widget_first_last_classes' );
...
function localedge_post_classes( $classes ) {
	global $wp_query;
...
add_filter( 'post_class', 'localedge_post_classes' );
...
function localedge_excerpt_more($more) {
	global $post;
	// edit here if you like
	return '...  <a href="'. get_permalink($post->ID) . '" title="'. __('Read', 'localedge') . get_the_title($post->ID).'">'. __('Read more &raquo;', 'localedge') .'</a>';
}

//This is a modified the_author_posts_link() which just returns the link.
//This is necessary to allow usage of the usual l10n process with printf().
function localedge_get_the_author_posts_link() {
	global $authordata;
